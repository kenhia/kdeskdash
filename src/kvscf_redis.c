...
int kvscf_redis_refresh(kvscf_instance_t *out, int max) {
    ...
    /* Collect the instance keys via a bounded SCAN loop (few keys — one host
     * today). guard caps iterations so a pathological cursor can't spin. */
    ...
    do {
        redisReply *r = redisCommand(g_kv.ctx, "SCAN %llu MATCH %s COUNT %d",
                                     cursor, KV_SCAN_MATCH, KV_SCAN_COUNT);
        if (!r) {
            g_reachable = false;
            return 0;
        }
        ...
        if (ks->type == REDIS_REPLY_ARRAY) {
            for (size_t i = 0; i < ks->elements && nkeys < KV_MAX_HOSTS; i++) {
        ...
        freeReplyObject(r);
    } while (cursor != 0 && ++guard < 16 && nkeys < KV_MAX_HOSTS);

    /* GET + parse each host's JSON, merging into out[]. */
    int count = 0;
